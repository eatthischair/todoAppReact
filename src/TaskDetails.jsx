...
const TaskDetails = ({task, taskDetailsOpen, setTaskDetailsOpen, taskArr, setTaskArr, editTaskModalIsOpen, setEditTaskModalIsOpen, unfilteredTaskArr, setUnfilteredTaskArr, taskIndex}) => {
  ...
  const [deepCopyTask, setDeepCopyTask] = useState(JSON.parse(JSON.stringify(task)))
  ...
  function updateNestedData(newArr) {
    ...
    setTaskArr((prevState) =>
      prevState.map((item, index) =>
        index === taskIndex ? { ...item, checklistItemsCompletedIndices: newArr }
      : item
      )
    );
  }
...
return (
  ...
  <div class='task'>
      <h4>{task?.taskName}</h4>
      <div><CalendarMonthIcon></CalendarMonthIcon>Due Date: {task.date} {task.time}</div>
          <div><ArrowUpwardIcon></ArrowUpwardIcon>Priority: {task.priorityLevel}</div>
          <div><OpenWithIcon></OpenWithIcon>Complexity: {task.complexityLevel}</div>
      <div>Task Complete</div>
      <LinearProgress value={checklistClickedPercentage} variant="determinate" ></LinearProgress>
      <div class='checklistBox'>Checklist for Subtasks
      {task?.checklist?.map((item, index) =>
        <div onClick={() => handleChecklistClicked(index)} class='checklistTask'
        style={{
       ...
       <CheckIcon></CheckIcon></div>)}
        </div>
      </div>

    <div class='taskDetailsBtns'>
    <button onClick={() => setTaskDetailsOpen(false)}><UndoIcon></UndoIcon></button>
    <button><RepeatIcon></RepeatIcon></button>
    <button onClick={() => deleteTask()}><DeleteIcon></DeleteIcon></button>
    <button onClick={() => setEditTaskModalIsOpen(true)}><ModeEditIcon></ModeEditIcon></button>
    </div>

    <div>{editTaskModalIsOpen ? <AddTaskModal
