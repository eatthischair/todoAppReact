...
import LevelSelector from './CardComponents/LevelSelector';
...
const AddTaskModal = ({taskDetailsOpen, setTaskDetailsOpen, taskArr, setTaskArr, task, editTaskModalIsOpen, setEditTaskModalIsOpen, setDeepCopyTask, editTaskModalFromDetails, unfilteredTaskArr, setUnfilteredTaskArr}) => {
  ...
  console.log('addtaskmodal', unfilteredTaskArr, setUnfilteredTaskArr)
  ...
  const updatePriorityLevel = (num) => {
    // console.log('updatelevel num', num);
    setPriorityLevel(num);
  }
  const updateComplexityLevel = (num) => {
    // console.log('updatecomplexity', num);
    setComplexityLevel(num)
  }

  const saveTask = () => {
    // console.log('savetask', taskIndex)
    let taskObj = {
  ...
  return (
    ...
    <h1>Add New Task</h1>
      <h2>Task Name</h2>
      <input onChange={(e) => updateTaskName(e.target.value)} type="text" placeholder="Name of task..." value={taskInputVal}></input>
      <div>Level of Priority</div>

      <div class='levelContainer'>
      <LevelSelector onClick={updatePriorityLevel} priorityLevel={priorityLevel} taskPriority={task?.priorityLevel}></LevelSelector>
      </div>

      <div>Level of Complexity</div>
      <div class='levelContainer'>
      <LevelSelector onClick={updateComplexityLevel} complexityLevel={complexityLevel} taskComplexity={task?.complexityLevel}></LevelSelector>
      </div>

      <DateandTimeInput date={date} setDate={setDate} time={time} setTime={setTime} datePlaceholder={task?.date} timePlaceholder={task?.time}></DateandTimeInput>

      <AddChecklist checklist={checklist} setChecklist={setChecklist} currentItem={currentItem} setCurrentItem={setCurrentItem} checklistPlaceholder={task?.checklist || checklist}></AddChecklist>
